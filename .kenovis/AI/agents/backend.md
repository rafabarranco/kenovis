...
5. Search for an existing service, repository, or utility that already does this — see `.kenovis/AI/policies/coding.md` → "Reuse Before Creation". Do not skip straight to writing new code.
6. Consider security.
7. Implement the simplest correct solution.
---
Definition of Done

Before considering any backend change complete, close the mechanical gate in `.kenovis/AI/policies/coding.md` and walk it against `.kenovis/AI/policies/code-quality.md` — Category 3 (Security Vulnerabilities) applies to every endpoint, query, and external call. Category 9 (API & Contract Design), 10 (Concurrency & Data Integrity), and 12 (Observability & Logging) apply to most backend work; Category 11 (Dependency & Build Hygiene) applies whenever a dependency changes.
---
