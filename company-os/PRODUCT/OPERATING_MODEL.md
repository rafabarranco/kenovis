...
|---|---|---|---|---|
| 1 | Observes continuously; raises to the right role; role refines; becomes planned work | **Partial** — re-verified 2026-08-21 rather than carried over. Plan and persist work; route (OF-31), refine (OF-32, 2026-08-14) and observe (OF-33, 2026-08-15, DECISION-038) now all have a mechanism, none complete. `grep -c -i "refine" framework/commands/next.md framework/policies/documentation.md framework/agents/designer.md` → **10, 1, 2** (was 8, 1, 2); `grep -c -i "observe" framework/commands/next.md` → **5**, unchanged. "Continuously" is not architecturally achievable (DECISION-038, same reasoning as DECISION-037's §15); what exists is a bounded, unconditional step inside `/next`, scoped to document-weight drift only — not the fuller breadth this row names (inconsistencies, technical debt, missing work, opportunities), which stays open as OF-87. **New this pass:** DECISION-057's own second tiebreak stage (previous pass's fix) saturated a second time the same day it shipped — every genuinely-`Open` row tied again, and the id-only fallback re-selected the same row (OF-02) with each repeat pick still costing a full new paragraph. Traced directly rather than assumed a fourth stage was owed: the tiebreak was not actually starving any row — it already rotates fair attention across a day, a repeat tie is that rotation completing another lap. DECISION-059 declares lowest id the terminal stage (no further stage) and replaces a saturated no-op re-check's full paragraph with a compact one-line form, closing the pattern that was growing `OF-02`'s own row large enough to weigh on `PRODUCT/ROADMAP.md`'s own size (item 14). Verified live in the same round: the compact form applied to `OF-02` itself, in the roadmap. Still strengthens the *mechanism's* correctness, not its independent validation — refine and observe still each rest on rounds that authored their own fix judging that it holds (OF-30/Learning-031's standing caveat is unaffected) | 2026-08-21 | OF-33, OF-87 |
| 2 | ABSOLUTE PRIORITY #1 — nothing discovered may be lost | **Partial** — re-verified 2026-08-21 rather than carried over. The rule is loaded unconditionally (item 39), its trigger is the moment of discovery (OF-38, OF-28, OF-29, 2026-08-14), and `check_item_findings.py` gained a third mechanical check this round: DECISION-051 (2026-08-19) closed the check being blind to a finding born outside a closed item and the check going permanently inert once a roadmap is fully archived; DECISION-055 (2026-08-21) closes a third, different blind spot — a finding correctly recorded as `Scheduled — item N` silently going stale once item N actually closed, with nothing re-checking it (OF-100, five confirmed instances). The three fixes are complementary, not overlapping: the first two guard a finding from never being recorded or from being invisible to the check; the third guards an already-recorded finding from silently going wrong after the fact — a "nothing may be lost" gap the first two do not cover on their own. Still Partial rather than Present: the behavioural half remains unvalidated by construction (no session has run `/init-project`/`/adopt-project` end to end since, and the round that writes a fix is the worst judge of whether it holds, per OF-30/Learning-031) — three mechanisms now, still no behavioural instance of a round independently exercising any of them | 2026-08-21 | OF-38, OF-28, OF-29, OF-21, OF-61, OF-100 |
...
| 4 | The founder decides; Kenovis owns engineering awareness | **Partial** — re-verified 2026-08-21 rather than carried over. The constitution no longer contradicts this section (OF-58, 2026-08-14: `SYSTEM.md` and `CLAUDE.md` stopped denying AI autonomy unconditionally, and now scope it to what this document reserves to the owner). A second mechanism-level gap closed 2026-08-16 (OF-40/Learning-028): `policies/documentation.md` now forbids returning a founder's own supplied material to them as a queue row, the exact "Not:" branch this section states. **New this pass, a live instance rather than only a mechanism:** with the `Open Findings` queue internally exhausted (OF-02/OF-03/OF-04/OF-30/OF-64/OF-87 all non-actionable this round) and item 33 the only scheduled item left, `/next` assembled the queue-exhaustion fact and item 33's own already-stated Target as input and presented the resulting decision to the founder, rather than mentioning the gap and waiting for it to be noticed — the exact "Not:" branch this row's own text names. Still Partial, not Present: one instance is not a closed class (OF-30/Learning-031's standing caveat), and the row's other named residual — the founder's own model of this repository's layers differs from the tree, with nothing comparing them — is untouched by this pass | 2026-08-21 | OF-35, OF-42, item 40 |
| 5 | Thread isolation must not silo knowledge | **Partial** — re-verified 2026-08-21 rather than carried over. Reach fixed (item 39); the cadence is specified in Addendum A and still not itself enforced as a mechanism; OF-71 (2026-08-19) named the `Next:` pointer as where a blocked round's stop-and-reason goes. New this round: a distinct instance of the same class closed directly — the isolated-clone-before-first-edit discipline (Learning-054) previously survived only inside one AI tool's own private, per-session memory, invisible to any other thread or tool; a second live near-miss this round (almost re-doing OF-74/OF-75, caught only by re-verifying) is exactly what that entry's own Future action asked to watch for, and `framework/commands/next.md` Step 3 now states the discipline inline, citing Learning-054, reachable by any thread regardless of tool. Row does not move to `Present`: this closes one concrete instance of knowledge that was thread/tool-siloed, not the cadence-enforcement gap OF-38/item 42 still name | 2026-08-21 | OF-38, item 42, Learning-054 |
| 6 | The Product layer is the persistent memory | **Partial** — re-verified 2026-08-21 rather than carried over: `wc -c` over the three archives → **753,277 bytes** (`ROADMAP-ARCHIVE.md` 601,991 + `LEARNINGS-ARCHIVE.md` 96,761 + `CHANGELOG-ARCHIVE.md` 54,525), up from 693,866 — this round's own OF-85 fix moving item 41's own progress narrative plus its seven step-4 round narratives into `ROADMAP-ARCHIVE.md` verbatim, weight relocated rather than newly created, same shape as the previous check's OF-51/OF-62 growth. `grep -l "ARCHIVE" framework/commands/*.md framework/workflows/*.md` → still **0**, unchanged. Row does not move: a written instruction plus a third growing evidence instance is not the same claim as the discipline being exercised routinely, the same OF-30/Learning-031 caveat as before. | 2026-08-21 | OF-37 |
...
| 13 | No silent debt | **Present as a rule** — re-verified 2026-08-22 rather than carried over, and by direct behavioural instance rather than only by count: **3** rows sit genuinely `Open` today (OF-30, OF-64, OF-87 — down from 4 this morning and 11 on 2026-08-21; item 33/OF-02/OF-03 closed via DECISION-060, OF-04 rejected this round), each still carrying Pain, Frequency and Cost. This round is itself a second, distinct instance of the rule holding: OF-04 sat `Open` through five refinements without closing, each reaching the same conclusion without ever being dispositioned — this round read that repetition as the evidence it was and closed the row instead of re-confirming it a sixth time, the same "stop letting a stable finding sit silently open" shape OF-74/OF-75 were the prior instance of | 2026-08-22 | OF-32, OF-66 |
| 14 | The roadmap is the complete representation of known future work | **Partial** — re-verified 2026-08-21 rather than carried over: it is complete, and `wc -c PRODUCT/ROADMAP.md` → **246,193 bytes** against a 60 KB threshold (4.0×) — down from 291,059 bytes (4.7×) at this round's own start, via OF-85's fix (item 41's own progress narrative and its seven step-4 round narratives moved to the archive, each under its own citable heading rather than one umbrella). Three consecutive rounds have now moved this multiple in the same direction, from 6.1× to 4.0×. Still a different way of not being readable — the multiple keeps dropping but stays well over threshold, and no further named lever is currently queued (OF-51, OF-62 and OF-85 are all `Fixed`). Row does not move — completeness and readability are the same measured gap they were, only smaller. | 2026-08-21 | OF-32, OF-62, OF-85 |
| 15 | Core invariant: violating it must be difficult or impossible | **Partial, moved from Absent (2026-08-21, DECISION-056) rather than carried over.** "Impossible" is still ruled out architecturally — DECISION-010 (tool-agnostic) and DECISION-013 (no backend/runtime) leave no place to stand a runtime enforcement point. "Difficult" now genuinely holds for the one Installation that actually runs this: OF-19's bypass gap is fixed — `development`'s branch protection dropped an unsatisfiable solo-maintainer review requirement and, for the first time, wired the CI guards as `required_status_checks`; a routine `gh pr merge --rebase` (no `--admin`) is now blocked by red CI rather than advisory to it. `enforce_admins` stays `false` by design — an admin override remains a deliberate emergency escape hatch, so "difficult," not "impossible," is still the honest ceiling. Reach (item 37) is unchanged and is now the row's only open gap: this fix is this repository's own GitHub settings, not a framework mechanism `sync` delivers to any customer Installation, so it reaches exactly one Installation | 2026-08-21 | OF-19 (fixed), item 37 |
...
**Corrected 2026-08-21, found while updating row 15, not carried over:** re-counted directly off the table's own State column (each row's first bolded word) rather than restated — **four** `Present` (§8, §10, §11, §13), **eleven** `Partial`, **two** `Absent` (§3, §12). The previous "five/nine/three" tally predates this correction and was already stale before this round's own row-15 edit moved it further off — §15 itself just left `Absent`, which is one reason the count needed re-deriving rather than incrementing by hand. The two remaining absent rows — §3, §12 — are the ones that make this an organisation rather than a notebook.
