...
| 6 | The Product layer is the persistent memory | **Partial** — re-verified 2026-08-21 rather than carried over: `wc -c` over the three archives → **612,032 bytes** (`ROADMAP-ARCHIVE.md` 460,746 + `LEARNINGS-ARCHIVE.md` 96,761 + `CHANGELOG-ARCHIVE.md` 54,525), up from 450,149 — most of the growth is this round's own OF-51 fix moving twelve superseded round narratives into `ROADMAP-ARCHIVE.md` verbatim, weight relocated rather than newly created. `grep -l "ARCHIVE" framework/commands/*.md framework/workflows/*.md` → still **0**, unchanged. What changed since the last check: nothing structural this round — the row's own behavioural evidence (item 24) and structural fixes (item 22/DECISION-042) are unaffected by a Product-layer-only archive pass. Row does not move: a written instruction plus one growing evidence instance is not the same claim as the discipline being exercised routinely, the same OF-30/Learning-031 caveat as before. | 2026-08-21 | OF-37 |
...
| 14 | The roadmap is the complete representation of known future work | **Partial** — re-verified 2026-08-21 rather than carried over: it is complete, and `wc -c PRODUCT/ROADMAP.md` → **299,418 bytes** against a 60 KB threshold (4.9×) — down from 363,766 bytes (6.1×) at this round's own start, via OF-51's fix (twelve superseded round narratives archived, a real lever previously believed exhausted). Still a different way of not being readable: the multiple dropped for the first time on record, but stays well over threshold, with OF-62's own residual (33 uncompacted Fixed/Rejected queue rows, ~76 KB) now the named next lever. Row does not move — completeness and readability are the same measured gap they were, only smaller. | 2026-08-21 | OF-32, OF-62 |
