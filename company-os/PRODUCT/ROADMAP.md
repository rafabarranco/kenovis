...
44. SCHEDULED (2026-08-18, via `/architect`, founder-decided — **DECISION-043**) — the Product layer namespaces under one visible root directory, `company-os/`, in every Installation and in this repository's own tree. Re-root only, no internal renames: the seven elements (`COMPANY_OS.md`, `DECISIONS.md`, `PRODUCT/`, `DOMAIN/`, `ENGINEERING/`, `AUTOMATIONS/`, `AI/memory/`) move under `company-os/` so a brownfield customer's own files are never overwrite candidates and the Collision Guard's surface drops from seven generic names to one. Scope, measured at decision time: `framework/` prose (58 of 64 files, ~460 citation lines), the two setup commands (~96 references), the `product-layer/` template bundle, 11 CI guards, `DOMAIN/BUSINESS_RULES.md` RULE-INST-01's path list, `NON_EVIDENCE_ENTRIES` in `cli/src/domain/installation.ts`, this repository's own root layout, and a written migration note for the one pre-1.0 external Installation (`kenovis@0.3.0`). **Must land before 1.0.0** — after it, this is a breaking layout change for live Installations. Origin: OF-92. Validated when: a fresh `kenovis init` + `/init-project` run authors the Product layer under `company-os/`; a brownfield fixture repo owning its own `PRODUCT/ROADMAP.md` completes `/adopt-project` with zero overwrite-or-move-aside questions about customer files; all CI guards pass on this repository's migrated tree; and `grep -rE "(^|[^-a-zA-Z/])((PRODUCT|DOMAIN|ENGINEERING|AUTOMATIONS)/|COMPANY_OS\.md|DECISIONS\.md|AI/memory/)" framework/` finds no un-namespaced live citation.
...
Next (updated 2026-08-18, after item 41 §3/§12 re-verified, OF-61 refined, and item 44 scheduled via `/architect`/DECISION-043):

Closed and not repeated below: nothing closed this round — §3 and §12 stay `Absent` by design, correctly cited rather than fixed. The superseded `Next` ordering block this closes is `PRODUCT/ROADMAP-ARCHIVE.md`'s.

1. **New top candidate: confirm item 41's `Validated when` bar independently and close the item if it holds — in a fresh round, not this one.** This round found the bar apparently met (see above) but does not trust its own finding on a founder-flagged maximum-priority item; a session that did not author today's fixes should re-read `PRODUCT/OPERATING_MODEL.md` → Conformance directly, confirm all three `Absent` rows (§3, §12, §15) still carry a live finding or decision, and close item 41 with that as the evidence — or state plainly what still fails if one does not hold.
2. **Item 38 — `sync` names what it removed from `.kenovis/`, closes OF-01.** No dependency, ready to execute directly: real CLI code (`cli/src`), scoped and small. Take this if item 41's confirmation is a quick read rather than a full round's work, so the round still lands code.
3. **Item 44 (DECISION-043, founder-decided 2026-08-18) enters here** — the `company-os/` Product-layer namespacing. Ranked below item 41's confirmation and item 38's small code round, ahead of item 23: founder-decided with a hard sequencing bound (before 1.0.0) rather than founder-flagged maximum priority, and wide enough that it deserves a round of its own, not a shared one.
4. Item 23 — structurally unblocked (items 18-22 all resolved: 18-21 `DONE`, 22 `Rejected`), but its own text requires reassessing against items 18-22's measurements *before* building, not building directly. Rank behind items 41, 38 and 44 until that reassessment happens.
5. **Item 33 ranks last, unconditionally, per DECISION-041.** Unchanged. The release question inherits this: hold until item 33 finally runs or the roadmap empties, whichever Addendum C reaches first. **Item 44 must complete before the 1.0.0 that follows item 33** (DECISION-043).
6. **OF-78, OF-79, OF-83, OF-84, OF-85 and OF-87 stay deliberately unranked above the blocks above; this is the twenty-third round to say so about OF-78.** Unchanged reasoning. **OF-90 joins this set**, for the same reason: real, dimensioned, and not ready to jump ahead of item 41's confirmation on its first round in the queue.
...
Per Learning-023, checked before writing this pointer: this round added one finding and refined one row, so the figure below is read after this round's own edits.
---
