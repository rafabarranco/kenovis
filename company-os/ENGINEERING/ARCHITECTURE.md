...
- The CLI must write the Framework layer under `.kenovis/` in the target repository (`.kenovis/AI/`, `.kenovis/README.md`), never at repo root — except `CLAUDE.md` (stub loading `.kenovis/AI/SYSTEM.md`, unconditional) and each selected AI tool's own scaffolding (`.claude/commands/` for `claude`, the default), which stay at repo root because that is where the tool itself requires them. Which tools get scaffolding is `--tools`-selected, data-driven from `framework/tool-adapters/<id>/` — the CLI itself never branches on a tool's identity or hardcodes which ones exist. See DECISIONS.md DECISION-017, DECISION-046. The CLI must never overwrite, append to, or otherwise touch the target repository's own existing `README.md`, and must never fabricate one if none exists.
...
- Exception to the rule above: this repository's own root `README.md` and root `CLAUDE.md` are never written or overwritten by `kenovis init`/`add`/`sync`, even if one of those commands is ever run against this repository's own working tree. Both stay hand-authored at repo root — this repository is the Framework layer's origin and its own dogfooded product simultaneously, not a generic Installation, and its root README/CLAUDE.md carry real content (public landing page; repo-specific Role/Layers/Source-Of-Truth/graphify prose) a generated stub would discard. See DECISIONS.md DECISION-020.
- This repository's own `.kenovis/` is generated, gitignored, never hand-edited — the same as any customer Installation's. The real Framework-layer source lives at root `framework/` (`framework/agents/`, `framework/workflows/`, `framework/policies/`, `framework/commands/`, `framework/templates/`, `framework/SYSTEM.md`), kept apart from `AI/memory/` (Product layer, stays at root) so no directory mixes layers. `cli/scripts/bundle-framework-assets.mjs` reads `framework/` to produce the published bundle and, as a local convenience, mirrors that same output into this repository's own `.kenovis/` for inspection. Root `CLAUDE.md` still reads `.kenovis/AI/SYSTEM.md` — true symmetry with a customer's own bootstrap path — with a self-heal line ahead of it if `.kenovis/` has not been built yet. See DECISIONS.md DECISION-039.
---
...
True only here. `.kenovis/AI/` is a customer Installation's read-only copy of the framework — never its product, never a role's to own. In this repository, since DECISION-039, `.kenovis/` is exactly the same kind of thing: generated, gitignored, never hand-edited. What is this product's own deliverable, and what most rounds on this board edit, is root `framework/` — the real source `.kenovis/AI/` used to be before DECISION-039 relocated it. `.kenovis/AI/agents/cto.md`, the framework-layer file every Installation receives, stays silent about this on purpose: writing "you own `framework/`" there would ship a claim true of one repository and meaningless in every other one it reaches, which has no `framework/` directory at all.

So the ownership is recorded here instead, where DECISION-020's self-modification exception already lived and DECISION-039 continues it: **the CTO role owns findings and work whose destination is `framework/`** — the same role that already owns architecture, engineering direction and technical risk, extended to the product this specific company happens to build. (`PRODUCT/ROADMAP.md` OF-55, item 43.)
---
Database
