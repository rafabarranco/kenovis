...
**Root `CLAUDE.md` is repointed to read `framework/SYSTEM.md` directly, not `.kenovis/AI/SYSTEM.md`.** Root `CLAUDE.md` is already hand-authored and already exempt from every CLI write (DECISION-020), so this changes nothing about that exemption — it only changes which file the exemption's own content points a session at. The consequence is the one that matters most for adoption risk: a session's ability to bootstrap in this repository never depends on a build step having run. A fresh `git clone` with no `npm install` and no build still has a complete, readable `framework/`, because it is committed source, not generated output.
...
**Pointing `CLAUDE.md` at `framework/` directly, rather than requiring `.kenovis/` to be regenerated before a session can bootstrap.** The first framing of this option (in the `/architect` session that produced this ADR) proposed a self-heal instruction — "if `.kenovis/AI/SYSTEM.md` is missing, run the build first" — as a mitigation for the fresh-checkout risk. Reading `framework/SYSTEM.md` directly removes the risk instead of mitigating it: there is no scenario where a session can start and find the framework layer missing, because it is the same committed source it always was, only renamed and relocated. Simpler beats a well-mitigated version of the more complex option, per this document's own recurring standard (`.kenovis/AI/policies/architecture.md` → "Simplicity First").

**Existing citations do not need to be rewritten.** Every citation throughout `DECISIONS.md`, `PRODUCT/ROADMAP.md`, `AI/memory/learnings.md`, and the framework files' own cross-references to each other, that names a path like `.kenovis/AI/policies/documentation.md`, stays literally true after this decision: that is still where the file is generated to, and still what `sync` delivers to a customer under the same path shape. What changes is only where a session *edits* the file — `framework/policies/documentation.md`, not `.kenovis/AI/policies/documentation.md` — which is new instruction, not a rewrite of everything already written.
...
**Keep the self-heal instruction in `CLAUDE.md` ("if `.kenovis/` is missing, build first") instead of pointing `CLAUDE.md` at `framework/` directly.** Rejected once the direct-pointer option was recognized as available — it removes the failure mode this alternative only papers over, at no extra cost, since `CLAUDE.md` is already hand-authored and already exempt from every CLI write.

## Consequences

### Positive Impact

- Full symmetry: `.kenovis/` means "installed, generated, never hand-edited" everywhere, including here — no remaining exception anywhere in the product.
...
- A session's bootstrap in this repository no longer depends on any build step having run — strictly simpler than today, not merely different.
...
- 9 of this repository's own 10 CI guards hardcode a `.kenovis/AI` or `.kenovis` path and need repointing to `framework/`.
...
- Every session that edits framework-layer content from this point on edits `framework/**`, not `.kenovis/AI/**` — a workflow change with no precedent in this repository to fall back on if it turns out to add more friction than expected.
...
- Rewrite `cli/scripts/bundle-framework-assets.mjs`: `sourceAiDir` becomes `join(repoRoot, "framework")`; after bundling to `cli/dist/framework-assets/`, add a step mirroring that output into `<repoRoot>/.kenovis/` (`AI/` and `README.md`), gitignored, local-only.
- `.gitignore` gains an entry for `/.kenovis/`.
- Root `CLAUDE.md`: repoint from `.kenovis/AI/SYSTEM.md` to `framework/SYSTEM.md`.
...
- All 9 affected guards in `.github/scripts/`: change `.kenovis/AI` (or `.kenovis`) path references to `framework/`.
...
- Fresh-checkout smoke test: a clean clone, no `npm install`, no build — confirm a session can still read `framework/SYSTEM.md` via root `CLAUDE.md` with `.kenovis/` entirely absent.
...
- A session bootstraps successfully from a fresh clone with no build step run — the concrete claim Phase 1 exists to make true.
