...
npm publishing is wired up (`.github/workflows/publish.yml`): pushing a GitHub Release triggers CI to build, test, typecheck and `npm publish --provenance --access public` the package — never from a developer's local machine, per [ENGINEERING/SECURITY.md](../ENGINEERING/SECURITY.md) → Supply-Chain Security. First publish is live: [`kenovis@0.1.0`](https://www.npmjs.com/package/kenovis) shipped from the `v0.1.0` GitHub Release, with provenance. `npx kenovis init` now works against any external repository with no local setup.
