...
|---|---|---|---|
| 1 | Observes continuously; raises to the right role; role refines; becomes planned work | **Partial** — plan and persist work; route (OF-31), refine (OF-32, 2026-08-14) and observe (OF-33, 2026-08-15, DECISION-038) now all have a mechanism, none complete. `grep -c -i "refine" .kenovis/AI/commands/next.md .kenovis/AI/policies/documentation.md .kenovis/AI/agents/designer.md` → **5, 1, 2** (was 3, 1, 2); `grep -c -i "observe" .kenovis/AI/commands/next.md` → **5**, from 0. "Continuously" is not architecturally achievable (DECISION-038, same reasoning as DECISION-037's §15); what exists is a bounded, unconditional step inside `/next`, scoped to document-weight drift only — not the fuller breadth this row names (inconsistencies, technical debt, missing work, opportunities), which stays open as OF-87. Refine and observe each have one live instance and no independent evidence yet (OF-30/Learning-031) | OF-33, OF-87 |
| 2 | ABSOLUTE PRIORITY #1 — nothing discovered may be lost | **Partial** — the rule is loaded unconditionally (item 39). Its trigger is now the moment of discovery, not an event that never fires, and a fresh Installation's setup commands each close with a disposition step for the window before their destinations exist (OF-38, OF-28, OF-29, 2026-08-14). Still Partial rather than Present: no session has run `/init-project` or `/adopt-project` end to end since, so the behavioural half is unvalidated by construction, not by measurement — the round that wrote the fix is the worst judge of whether it holds | OF-38, OF-28, OF-29 |
...
| 6 | The Product layer is the persistent memory | **Partial** — `wc -c` over the three archives → **450,149 bytes**, up from 320,100; `grep -l "ARCHIVE" .kenovis/AI/commands/*.md .kenovis/AI/workflows/*.md` → still **0** (2026-08-15, DECISION-independent fix, OF-37). What changed: `.kenovis/AI/SYSTEM.md` → "Context Loading Rules" now says when to open an archive, the same discipline it already required of a `DECISIONS.md` body. **New, 2026-08-16 (item 24):** that same discipline now has real behavioural evidence for decision bodies — a fresh, uninformed subagent run against a seeded fixture correctly used a decision's body content (not its index summary) to reach an answer the index alone could not have produced, checked against the artifact it wrote, not just its self-report. The row still does not move — a written instruction is not the same claim as a round actually following it (OF-30/Learning-031), archives and decisions each have exactly one instance, and no command or workflow file itself opens either automatically. OF-43 (previously cited here) is Fixed and no longer an open gap. **New, 2026-08-16 (item 22 / DECISION-042):** the targeted-read mechanism is now named explicitly in `SYSTEM.md` rather than assumed, and `DECISIONS.md`'s threshold answer is correctly classified ("index-bounded," checked by a guard) instead of citing a since-rejected item as a placeholder — structural, not behavioural; the behavioural count is unchanged | OF-37 |
...
| 12 | The role that owns the responsibility processes the discovery | **Absent** — a finding can now name its owning role (DECISION-035, `policies/documentation.md` 3.10) and this repository's own CTO ownership of `.kenovis/AI/` is recorded (`ENGINEERING/ARCHITECTURE.md` 1.7), but naming an owner is not the owner processing anything: `grep -rn "Open Findings\|disposition" .kenovis/AI/agents/*.md` → still **0** across **12** agent files, unchanged, since the fix lives in the disposition rule rather than in the roster itself | OF-31, OF-55, OF-32 |
