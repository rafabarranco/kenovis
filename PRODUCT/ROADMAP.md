...
Audited before scoping any option, live in the same session: `find .kenovis -type f | wc -l` → 64, `git ls-files .kenovis | wc -l` → 64, all tracked, nothing stray; `grep -n "kenovis" .gitignore` → one comment, no ignore rule. `grep -l "\.kenovis" .github/scripts/*.py | wc -l` → **9** of 10 guards hardcode a `.kenovis` path reference and need repointing to `framework/` in Phase 2 — the real size of this item's remaining work, not the file-move alone.
