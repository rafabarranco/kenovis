...
Next (updated 2026-08-16, after OF-14 closed and OF-51 refined — item 22's own rejection, DECISION-042, also landed between rounds):

Closed and not repeated below: **OF-14** (Fixed — the size guard's failure branch exercised and confirmed, first time ever). **item 22** (Rejected via DECISION-042, landed in the round immediately before this one — not this round's own work, but it changes what "stays behind OF-14" means, so it is not repeated as a live dependency below). **DECISION-042** itself is not `/next` work. The superseded `Next` ordering block this closes is `PRODUCT/ROADMAP-ARCHIVE.md`'s.

1. **New top candidate: item 41 — founder-flagged *maximum priority* (2026-08-13), still `SCHEDULED`, and apparently stale rather than genuinely blocked.** Found while writing this pointer, not executed this round — this round's own objective was OF-14, and picking up item 41 instead would be exactly the undisclosed departure DECISION-030 forbids. What was found: all four of item 41 target step 4's named sub-gaps are closed (§2 via OF-38/OF-28/OF-29, §12/§1 via OF-31/OF-32, §15 via OF-44/DECISION-037, §1/§16 via OF-33/DECISION-038, §6/§9/§17 via OF-37, §4 via OF-41/item 43/OF-35-DECISION-040) — read back via `grep -n "^38\.\|^41\." PRODUCT/ROADMAP.md` and cross-checked against the Open Findings queue's own dispositions, not assumed from the item's own narrative. What is **not** yet re-verified: item 41's own `Validated when` bar — "no row reading `Absent` that is not accompanied by a recorded decision to leave it so" — against `PRODUCT/OPERATING_MODEL.md` → Conformance's current §3 and §12 rows, which as of the table's last full pass (2026-08-14) still cite OF-35 and OF-31/OF-55/OF-32 respectively as open reasons; all of those findings are now `Fixed`, so the rows' own citations are stale in the same shape OF-79 already names (a conformance row re-verified only when a round happens to serve its section). The next round's first move on this item should be exactly that re-verification, not a re-read of item 41's already-settled target list.
2. **Item 38 — `sync` names what it removed from `.kenovis/`, closes OF-01.** No dependency, ready to execute directly: real CLI code (`cli/src`), scoped and small. Take this if item 41 turns out to be a `Present`/decision-recording round rather than one with implementation work, so the round still lands code.
3. Item 23 — now structurally unblocked (items 18-22 all resolved: 18-21 `DONE`, 22 `Rejected`), but its own text requires reassessing against items 18-22's measurements *before* building, not building directly. Rank behind items 41 and 38 until that reassessment happens.
...
5. **OF-78, OF-79, OF-83, OF-84, OF-85 and OF-87 stay deliberately unranked above the blocks above; this is the twenty-second round to say so about OF-78.** Unchanged reasoning. OF-79 is the same defect item 41's own staleness (point 1 above) is a fresh instance of — take them together if either is picked up.
...
Per Learning-023, checked before writing this pointer: this round closed one finding and refined one row, so the figure below is read after this round's own edits.
---
