...
Next (updated 2026-08-16, after item 24 closed and OF-43/OF-89 resolved — a founder decision, DECISION-041, landed between rounds and changes how item 33 ranks going forward):

Closed and not repeated below: **item 24** (DONE — decision-body citation validated live, positive result). **OF-43** (Fixed, refined into closure — its own ask already existed as the Conformance table). **OF-89** (Fixed via DECISION-041 — founder answered directly). **DECISION-041** itself is not `/next` work, it is standing guidance the ranking below already applies. The superseded `Next` ordering block this closes is `PRODUCT/ROADMAP-ARCHIVE.md`'s.

1. **New top candidate: OF-14 — exercise `check_document_size.py`'s failing case with a fixture for `AI/memory/learnings.md`.** Cost is one temporary fixture (grow a governed document past 60 KB, confirm the guard fails, revert), not a code change. Re-check the premise on pickup per Learning-023 — `AI/memory/learnings.md`'s current size may have moved since this row was written.

2. Item 22 stays behind it — requires `/architect` and an ADR before any file is touched, per DECISIONS.md restructuring's own blast-radius warning; not startable directly from `/next`.

3. **Item 33 ranks last, unconditionally, per DECISION-041** — not "held pending a customer-facing fix" as the previous pointer read, but explicitly deprioritized behind every other scheduled item and finding until the roadmap is otherwise done, per the founder's own words. The release question inherits this: hold until item 33 finally runs (which by design is late) or the roadmap empties, whichever the founder's own Addendum C reaches first.

4. **OF-78, OF-79, OF-83, OF-85 and OF-87 stay deliberately unranked above the blocks above; this is the twenty-first round to say so about OF-78.** Unchanged reasoning.

Not `/next` work, unchanged: item 32 (founder input). Item 33 needs an external party and now also ranks last by founder instruction. Item 25 (the numbered roadmap item) is rejected as originally scoped.

Per Learning-023, checked before writing this pointer: this round closed one item and two findings and refined one row, so the figure below is read after this round's own edits.
---
