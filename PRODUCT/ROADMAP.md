...
Next (updated 2026-08-18, after item 41 §3/§12 re-verified and OF-61 refined):
...
3. Item 23 — structurally unblocked (items 18-22 all resolved: 18-21 `DONE`, 22 `Rejected`), but its own text requires reassessing against items 18-22's measurements *before* building, not building directly. Rank behind items 41 and 38 until that reassessment happens.
4. **Item 33 ranks last, unconditionally, per DECISION-041.** Unchanged. The release question inherits this: hold until item 33 finally runs or the roadmap empties, whichever Addendum C reaches first.
