...
1. **Item 41 step 1 with OF-50** — rank the specification (OF-39) and put it on the session-initialization path. **This is a founder call, and it is now the top of the board.** Under `commands/next.md` 2.6 Step 3 the next round does not plan it and does not skip past it: it presents the decision with the input already on disk, records that it stopped and on what, and stops. The input is both Source Of Truth Hierarchies — `CLAUDE.md`'s and `.kenovis/AI/SYSTEM.md`'s — which disagree with each other and neither of which lists `PRODUCT/OPERATING_MODEL.md` at any rank. The decision is which hierarchy governs and where the operating model sits in it. Nothing below can be checked against a specification whose rank is undefined, which is why it is first rather than cheap.
2. **Item 41 step 2** — the conformance table into `PRODUCT/OPERATING_MODEL.md`. This is where OF-43 dies and where every later round gets its criterion.
3. **Item 42 parts 4-5**, then **item 41 step 3** (`COMPANY_OS.md` reconciliation), then **item 41 step 4**'s gap sequence beginning with §2 (OF-38 + OF-28 + OF-29).
