...
39. SCHEDULED — the findings rule is carried by the layer every session loads, not by one command. Closes OF-26, and is the vehicle for OF-21's first output.
...
Next (updated 2026-08-13, after item 37's second round): **item 37, round 3** — `check_decision_index`, `check_document_size`, `check_learning_promotions`.
...
**Item 39 does not wait for item 37.** It adds no guard — that is its explicit scope limit — so DECISION-026's and OF-21's prohibition does not touch it, and it is the only item on the board whose absence keeps costing findings in every thread that runs while it is queued. Founder raised it on 2026-08-13 as the defect that makes the product not do the one thing it exists to do; treat it as ahead of item 37's round 3.
