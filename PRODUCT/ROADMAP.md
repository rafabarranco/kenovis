...
3. **Reconcile `COMPANY_OS.md` with §1** — OF-36's open half. Vision says *augmentation* ("the execution capacity of a full specialized organization"); §1 says *replacement* ("without requiring a conventional human development team"). Founder call, one sentence, and every measurement downstream inherits it.
...
Next (updated 2026-08-13, after the `/analyze` on the founder's supplied usage model — supersedes the ordering written an hour earlier in this same section):

0. **OF-45, and it is not a `/next` round.** `PRODUCT/OPERATING_MODEL.md` is untracked. The specification everything below is ordered around exists in one working tree. Commit it before opening the next thread; `policies/git.md` reserves that for explicit human instruction, which is why it is step 0 and not step 1.
1. **Item 42 part 1 — the archive pass (OF-51/OF-23).** 131,511 bytes, 76% of every thread's mandatory reading, now multiplied by one thread per roadmap step. Written procedure, run twice, cheapest change on the board.
2. **Item 42 parts 2-3 (OF-46, OF-47).** Both fire on the founder's very next `/next`: the pointer is written and never read, and the top item is a founder call the command has no branch for. Doing these before item 41 step 1 means the round that hits that founder call handles it correctly instead of demonstrating OF-47.
3. **Item 41 step 1 with OF-50** — rank the specification (OF-39) and put it on the session-initialization path. Founder call; input is on disk.
4. **Item 41 step 2** — the conformance table into `PRODUCT/OPERATING_MODEL.md`. This is where OF-43 dies and where every later round gets its criterion.
5. **Item 42 parts 4-5**, then **item 41 step 3** (`COMPANY_OS.md` reconciliation), then **item 41 step 4**'s gap sequence beginning with §2 (OF-38 + OF-28 + OF-29).
...
Everything below this line stays scheduled and is now behind item 41: item 37 round 3, OF-25, item 24, OF-14, item 22, the release. The reasoning that ordered them has not changed and is kept verbatim; only their rank has.
