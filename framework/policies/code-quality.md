...
`.kenovis/AI/policies/coding.md` defines philosophy: how to think about code.
...
- `security` — as the concrete companion to its own principle-based checklist in `.kenovis/AI/policies/security.md`, specifically Categories 3 and 4.
...
Before declaring any code task complete, and again during review. See `.kenovis/AI/policies/coding.md` → "Definition of Done — Mechanical Gate" for how this document is wired into the actual workflow.
...
- Tests assert on behaviour, not on internal implementation details (see `.kenovis/AI/policies/testing.md`).
...
- A dependency added to solve a problem the standard library already solves is questioned before being kept (see `.kenovis/AI/policies/coding.md` → Dependencies).
...
- Every log line answers what happened and why — not just "error" or "done" with no context (see `.kenovis/AI/policies/coding.md` → Logging).
- Logs never contain secrets, tokens, passwords, or personal data — this is a security requirement, not a style preference (see Category 3/4 above and `.kenovis/AI/policies/security.md`).
...
1. Run the project's real linter/formatter/type-checker (see `.kenovis/AI/policies/coding.md` → Definition of Done). Fix everything it reports.
