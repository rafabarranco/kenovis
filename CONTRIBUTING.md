...
- **Agents** (`.kenovis/AI/agents/`): a specialized role, tool-agnostic, reusable across any product built on this framework.
- **Workflows** (`.kenovis/AI/workflows/`): a repeatable process (feature, bugfix, release, etc.) that doesn't assume a specific tech stack or domain.
- **Policies** (`.kenovis/AI/policies/`): non-negotiable rules that hold regardless of product — e.g. "never trust the client," not "always use PostgreSQL."
- **Commands** (`.kenovis/AI/commands/`): entry points that orchestrate agents/workflows.
- **Templates** (`.kenovis/AI/templates/`): document skeletons (ADR, decision, feature plan, etc.).
...
Files under `.kenovis/AI/agents/`, `.kenovis/AI/workflows/`, `.kenovis/AI/policies/`, `.kenovis/AI/commands/`, and `.kenovis/AI/templates/` carry a `Version: X.Y` header. Keep it honest:
