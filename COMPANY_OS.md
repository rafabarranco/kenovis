<!-- PROJECT-SPECIFIC: this product's own context, not framework. Authored by /init-project or /adopt-project; kenovis sync never overwrites it. -->

# COMPANY_OS.md

Company Operating System

Version: 2.2

Last updated: 2026-08-14 — "Knowledge Hierarchy" stopped stating a third Source Of Truth ordering and became "What Each Document Defines" (DECISIONS.md DECISION-032, PRODUCT/ROADMAP.md OF-73).

Previously: 2026-08-13 — Company Vision restated against PRODUCT/OPERATING_MODEL.md §1 (DECISIONS.md DECISION-028).

---

# Purpose

This document defines the operating principles, strategic vision and decision framework of the company.

It is the highest-level context document of the organization.

All humans and AI agents working on this company must understand and respect these principles.

The objective is not only to build software.

The objective is to build a sustainable, valuable and scalable company.

---

# Company Vision

Kenovis lets a product be built, maintained, evolved and supported without a conventional human development team. It replaces the team — the specialized roles, the engineering process, the institutional memory that normally dies with the people who held it — while the human who owns the product keeps product direction, strategy, business decisions and final approval (DECISIONS.md DECISION-028, PRODUCT/OPERATING_MODEL.md §1 and §4). The company builds and distributes the Kenovis AI-OS itself — a documented operating system of specialized AI agents, workflows, policies and decision discipline — starting with software development teams, and eventually extending the same operating model to other professional practices (legal, accounting, and beyond).

---

# Company Thesis

Most AI coding tools sell raw generation speed on top of chaos: code appears faster, but without the operating discipline a real specialized organization has — product strategy, architecture rigor, security review, documented decisions. Kenovis wins by selling the discipline itself, not just the generation. The product is a company constitution, a decision log, a domain model, enforced policies, and a roster of specialized AI agent roles that any team can install directly into their own repository — and Kenovis is built and run on that exact system, so every improvement is proven on itself before it reaches a customer.

---

# Initial Market Strategy

Software developers and small development teams. This segment is already fluent in AI-assisted and agentic tooling, adopts CLI/repo-based products with the least friction, can evaluate the framework's engineering rigor firsthand, and gives the fastest, most technical feedback loop. Kenovis also already dogfoods the framework on itself — this repository is a live instance of the product — so this segment can be served from day one without inventing a separate workflow.

---

# Long-Term Market Vision

The same operating pattern — constitution, decision log, domain model, policies, and a roster of specialized AI agents — extends to other professional practices where small teams need the leverage of a full specialized organization: accounting/gestoría, legal/abogacía, and other services-based practices. What stays constant across verticals: pairing a human owner with a documented, disciplined AI team, and the mechanics of the operating system itself (Framework layer). What changes per vertical: the specific agent roster, domain vocabulary and policies (Product layer, per-vertical).

---

# Ideal Customer Profile

A small team or solo builder shipping a real product or service who feels the absence of dedicated roles — product manager, architect, security reviewer, QA — but can't yet justify hiring for them, and who is comfortable adopting a CLI/repo-based tool rather than requiring a hosted dashboard from day one.

---

# Market Exclusions

Large enterprises with existing specialized teams and procurement processes, at least initially. Teams looking for a no-code/low-code page builder rather than an engineering operating system. Anyone wanting a chatbot wrapper without the documentation-and-decision discipline that is the actual product.

---

# Company Principles

The company operates under these principles:

## Customer Problems Over Feature Ideas

We build solutions for real problems.

A feature without customer value should not exist.

---

## Simplicity Over Complexity

The best solution is the simplest solution capable of solving the problem correctly.

Complexity requires justification.

---

## Distribution Over Technology

Technology is important.

However, customer access, trust and relationships create stronger advantages.

---

## Quality Over Short-Term Speed

Speed matters.

But building the wrong thing quickly is wasted effort.

---

## Long-Term Thinking

Every important decision should consider:

- Future scalability.
- Maintainability.
- Customer trust.
- Business sustainability.

---

## Ownership

Every person and AI agent working on the company should act with responsibility and ownership.

---

# Product Philosophy

Every feature must make it faster or safer for a small team to operate with the rigor of a full specialized organization. Measured by:

- Time from install to a completed, real (non-placeholder) product-layer.
- Consistency of documented decisions versus undocumented, ad hoc choices.
- Cases where the framework caught a missed policy, security gap or undocumented decision that plain AI-assisted coding would have missed.

Features without measurable value should not be built.

---

# Business Philosophy

The company prioritizes:

- Sustainable growth.
- Customer retention.
- Healthy margins.
- Product quality.
- Customer trust.
- Operational efficiency.

The objective is not growth at any cost.

The objective is building a valuable company.

---

# AI-Native Company Model

The company operates using an AI-native operating model.

AI is an operational multiplier.

AI supports:

- Software development.
- Product analysis.
- Documentation.
- Research.
- Marketing.
- Customer support.
- Internal operations.

AI reduces execution costs and increases speed.

The company is AI-native.

The product is software-first.

AI does not replace product value.

The product must create value independently.

---

# Company Operating Model

Initial operating model:

Human Leadership

+

AI Agents

+

External Specialists When Required


AI agents support:

- Engineering.
- Product management.
- Design.
- Marketing.
- Finance.
- Legal analysis.
- Customer success.


Human specialists are introduced when professional responsibility is required.

Examples:

- Legal validation.
- Tax decisions.
- Accounting.
- Brand identity.
- Advanced design review.

---

# What Each Document Defines

Each document in the company knowledge system answers a different question:

`PRODUCT/OPERATING_MODEL.md` — what this product is for, stated by its owner.

COMPANY_OS.md — why we exist, where we compete, how we think, how we decide.

DECISIONS.md — what was decided, and why.

PRODUCT/ — what we build, for whom, and the product direction.

DOMAIN/ — business concepts, rules, meaning.

ENGINEERING/ — technical implementation, architecture, infrastructure.

`.kenovis/AI/` — how AI agents operate, how work is executed, how decisions are supported.

**This list states what each document is for. It states no precedence.** When two of them conflict, the ordering that resolves it is the Source Of Truth Hierarchy in `.kenovis/AI/SYSTEM.md`, which is the only one.

This section used to be a "Knowledge Hierarchy" with an ordering of its own, and that ordering was a third one: it ranked PRODUCT/ above DOMAIN/ — the inverse of what DECISION-031 settled — and omitted DECISIONS.md entirely. It survived the round that reconciled the other two because a duplicate is found by comparing the copies you already suspect, and nothing enumerates them (`AI/memory/learnings.md` Learning-034, `PRODUCT/ROADMAP.md` OF-73). See DECISIONS.md DECISION-032.

---

# Decision Framework

Every important decision must answer:

## Customer Value

Does this solve a real customer problem?

---

## Business Value

Does this improve:

- Revenue?
- Retention?
- Customer satisfaction?
- Competitive advantage?

---

## Simplicity

Is this the simplest possible solution?

---

## Scalability

Will this decision help or hurt the future company?

---

## Trade-offs

What are we gaining?

What are we sacrificing?

---

If a decision creates unnecessary complexity, avoid it.

---

# Product Development Principles

The company values:

- Simplicity.
- Reliability.
- Maintainability.
- Speed.
- Quality.

Development speed matters.

However:

Fast development of the wrong thing is wasted effort.

Always prioritize:

Problem validation

>

Product design

>

Implementation

---

# Distribution Strategy

CLI/template distribution through the software developer community: open-source visibility, word of mouth among developers already using agentic coding tools, and Kenovis's own dogfooded repository as a public reference implementation of the product working on itself. No paid acquisition, no sales team, in the initial phase.

Customer satisfaction is the primary marketing channel.

---

# Customer Relationship Philosophy

Customers are partners in product development.

The company must:

- Listen carefully.
- Observe workflows.
- Understand frustrations.
- Prioritize real problems.

Customer feedback directly influences product direction.

---

# Competitive Advantages

Dogfooding: Kenovis is built and run using its own framework, so every improvement is validated internally before it reaches a customer. Zero central infrastructure to scale: the framework compounds across every customer install without Kenovis operating a backend. Documentation-as-memory discipline (DECISION-009, DECISION-011) compounds each customer's own domain and product knowledge inside their own repository — a switching cost the customer accumulates themselves, not one Kenovis imposes.

---

# What The Company Will NOT Become

A generic AI chatbot or general-purpose coding assistant competing purely on code-generation speed. A heavyweight SaaS platform with mandatory hosted infrastructure before the open-core framework itself has proven value. A project-management tool.

---

# AI Agent Instructions

Any AI agent working on this company must:

- Understand company vision before suggesting solutions.
- Consider customer value before implementation.
- Challenge unnecessary complexity.
- Prefer simple scalable approaches.
- Think strategically.
- Consider business impact.
- Respect company principles.

AI agents are not code generators.

They are collaborators helping build the company.

---

# Definition of Success

Success is not measured by:

- Number of features.
- Lines of code.
- Technical complexity.

Success is measured by:

- Organizations actively using the platform.
- Time saved for users.
- Operational problems solved.
- Revenue generated.
- Customer retention.
- Customer trust.

---

# Final Principle

Build less.

Understand more.

Solve painful problems.

Create a company, not just software.
